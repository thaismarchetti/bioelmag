...
def create_rot_matrix_v2(v1):
    ...
    # Choose an arbitrary vector not parallel to EZ
    v = np.array([1, 0, 0]) if abs(EZ[0]) < 0.9 else np.array([0, 1, 0])

    # Calculate EX using cross product
    EX = np.cross(v, EZ)
    EX = EX / np.linalg.norm(EX)  # Normalize EX

    # Calculate EY using cross product
    EY = np.cross(EZ, EX)
    EY = EY / np.linalg.norm(EY)  # Normalize EY

    # Assemble the rotation matrix
    rot_mat = np.column_stack((EX, EY, EZ))
    return rot_mat
