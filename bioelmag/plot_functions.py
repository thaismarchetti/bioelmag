...
def plot_psd(raw, picks="all", exclude=(), sqrt=True, scalling=1,
    ...
    f, Pxx_den = signal.welch(raw_data, raw.info["sfreq"], nperseg=nperseg)
    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1)
    ax.set_xscale('log')
    ax.set_yscale('log')
    if sqrt:
        plt.plot(f, np.sqrt(Pxx_den.T), linewidth=0.5)
    else:
        plt.plot(f, Pxx_den.T, linewidth=0.5)
...
def plot_magnetometers31(subject_dir, name, evoked, magnetometer_number,
    ...
    surface1 = os.path.join(subject_dir, name, 'bem', 'watershed',
                            name + '_inner_skull_surface')
    surface2 = os.path.join(subject_dir, name, 'bem', 'watershed',
                            name + '_outer_skull_surface')
    surface3 = os.path.join(subject_dir, name, 'bem', 'watershed',
                            name + '_outer_skin_surface')
    ...
    for j, i in enumerate(content):
        if (i.split()[0].isdigit()):
            if (magnetometer_number == int(i.split()[1]) and
               accuracy == int(i.split()[2])):
    ...
    p1 = plot_sensors_pyvista1(
        surfaces, sensors=sensors, elements=elements, arrow_color="black",
        grad=magnetometer_gradiometer)
...
def plot_sensors_pyvista1(surfaces, sensors, sensors2=[], elements=[],
    ...
    for cunt, element in enumerate(elements):
        ...
        if len(element) == 1:
            ...
            p.add_mesh(sphere, color="black")
            arrow = pv.Arrow(start=pts[i, 0:3], direction=sensors[j, 3:6],
                             scale=20)
            p.add_mesh(arrow, color=arrow_color)
        if len(element) == 4:
        ...
        if len(element) == 8:
            faces = np.array(
                [[4, 0, 1, 3, 2], [4, 4, 5, 7, 6], [4, 2, 3, 7, 6],
                 [4, 0, 1, 5, 4], [4, 1, 3, 7, 5], [4, 0, 2, 6, 4]])
            mesh = pv.PolyData(pts, faces)
            p.add_mesh(mesh, color="red")
            arrow = pv.Arrow(start=np.mean(pts, axis=0),
                             direction=sensors[j, 3:6], scale=20)
            p.add_mesh(arrow, color=arrow_color)
    ...
    for i, surface in enumerate(surfaces):
        ...
        p.add_mesh(polygon, color=gray1, opacity=opacities[i] - 0.2)
